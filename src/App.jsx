    ...
    const handler = e => {
      if ((e.ctrlKey || e.metaKey) && e.key === 'n') {
        ...
        if (userCan('createDemand', user?.role)) {
          ...
          setTimeout(() => {
            document.getElementById('btn-new-demand')?.click()
          }, 100)
        }
      }
      ...
      if ((e.ctrlKey || e.metaKey) && e.key === 'i') {
        ...
        if (userCan('stockIn', user?.role)) {
          ...
          setTimeout(() => {
            document.getElementById('btn-stock-in')?.click()
          }, 100)
        }
      }
    }
...
function AppContent() {
  ...
  const complaintMatch =
    window.location.pathname.match(/^\/complaint\/([^/]+)\/?$/)

  if (complaintMatch) {
    return (
      <ComplaintPublic
        token={decodeURIComponent(complaintMatch[1])}
      />
    )
  }
  ...
  if (loading && !dataLoaded) {
    return (
      <LoadingScreen
        message={`Loading ${user.branch_name ?? 'branch'} data…`}
      />
    )
  }

  // Master users can operate without a branch.
      ...
      >
        ...
        >
          ...
          >
            ...
            table to a valid branch, then refresh the page.
          </p>

          <button
            onClick={async () => {
              const { authApi: a } = await import('./lib/api')
              await a.logout()
            }}
            style={{
              padding: '10px 28px',
