...
<<<<<<< HEAD
    ...
    if (currentUser?.role === 'Admin' && ['Master','Developer'].includes(u.role)) {
...
<<<<<<< HEAD
      ...
      <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap', marginBottom:14 }}>
        ...
        {['All','Active','Inactive'].map(status => (
          <Btn key={status} variant={userStatusFilter === status ? 'primary' : 'outline'} onClick={() => setUserStatusFilter(status)} style={{ padding:'7px 12px', fontSize:11 }}>
            {status}
          </Btn>
        ))}
      </div>
