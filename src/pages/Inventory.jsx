...
const UNITS = ['pcs', 'kg', 'g', 'L', 'ml', 'box', 'carton', 'bag', 'bottle', 'pack']
...
export default function Inventory() {
    ...
    return withActionLock(async () => {
      ...
      try {
        ...
        if (editingItem) {
          ...
          const { data, error } = await inventoryApi.create({
            itemData: payload, branchId: user.branch_id, userId: user.id, userName: user.name,
          })
  ...
  const canAdd = userCan('addInventory', user?.role) || true
    ...
    <div className="animate-fade-in">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div>
          ...
          <p style={{ fontSize: 12, color: theme.textMuted }}>Manage stock, thresholds, and suppliers</p>
        </div>
        {canAdd && (
          <Btn id="btn-add-item" variant="primary" onClick={openAddModal}>
            <Ic n="Plus" size={14} color="white" /> Add Item
          </Btn>
        )}
      </div>
      ...
      <Card style={{ padding: 0, overflow: 'hidden' }}>
        {loading ? (
          ...
          <EmptyState icon="Package" title="No inventory items yet" message="Click Add Item to start building your inventory." />
        ) : (
