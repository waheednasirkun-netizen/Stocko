...
export default function Dashboard() {
  ...
  const pendingRequests = useMemo(() =>
    requests.filter(d => d.status === 'Pending').slice(0, 5),
    [requests]
  )
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
          ...
          <Card>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
              <h3 style={{ fontSize:15, fontWeight:700, color:theme.text }}>Pending Requests</h3>
            ...
            {pendingRequests.length === 0
              ? <div style={{ fontSize:13, color:theme.textMuted, textAlign:'center', padding:'20px 0' }}>No pending requests 🎉</div>
