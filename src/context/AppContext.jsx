...
<<<<<<< HEAD
    ...
    if (userProfile?.role === 'Master') {
      try {
        ...
        console.warn('[AppContext] Could not load global Master branches:', err)
      }
    }
...
<<<<<<< HEAD
  ...
  useEffect(() => {
    ...
    const poll = window.setInterval(() => { void loadNotifications() }, 20000)
...
<<<<<<< HEAD
      ...
      // Fetch accessible branches. Master is global, but the existing Stocko screens
      // are branch-oriented, so give Master the first branch as the initial view;
      // the normal branch selectors still allow switching across every branch.
      ...
      if (userBranches.length > 0) {
        ...
        const sessionUser = restoredUser.role === 'Master'
          ? { ...restoredUser, branch_id: defaultBranch.id, branch_name: defaultBranch.name }
          : restoredUser
        setUser(sessionUser)
