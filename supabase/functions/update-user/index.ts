...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (role === MASTER_ROLE && !isDeveloper) {
    ...
    if (target.role === MASTER_ROLE && !isDeveloper) {
      return json({ success: false, error: "Only a Developer can modify a Master account" }, 403);
    }
    if (!ASSIGNABLE_ROLES.has(role) && !(role === MASTER_ROLE && isDeveloper) && !isEditingExistingDeveloper) {
