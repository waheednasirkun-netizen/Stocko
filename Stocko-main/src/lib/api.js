...
// Retries a Supabase call on transient rate-limit / network errors
// (HTTP 429, or a fetch-level failure) with short exponential backoff.
// Auth-time queries (profile/branch lookups) are the ones most likely to
// collide with other requests fired at login, so this keeps a transient
// 429 from permanently stranding the user on the login screen.
async function withRetry(queryFn, { retries = 3, baseDelayMs = 400 } = {}) {
  let lastResult
  for (let attempt = 0; attempt <= retries; attempt++) {
    lastResult = await queryFn()
    const status = lastResult?.error?.status ?? lastResult?.error?.code
    const isRateLimited =
      status === 429 ||
      status === '429' ||
      /too many requests|rate limit/i.test(lastResult?.error?.message ?? '')
    if (!lastResult.error || !isRateLimited || attempt === retries) {
      return lastResult
    }
    const delay = baseDelayMs * 2 ** attempt + Math.random() * 150
    console.warn(`[api] rate-limited, retrying in ${Math.round(delay)}ms (attempt ${attempt + 1}/${retries})`)
...
export const authApi = {
  async login(email, password) {
    ...
    // NOTE: signInWithPassword() above already establishes and persists the
    // session, and already fires an 'SIGNED_IN' onAuthStateChange event.
    // Calling supabase.auth.setSession() again here (as this used to do)
    // fires a SECOND auth event for the same login, which caused the
    // AppContext auth listener to run its whole profile/branch/data-loading
    // cascade twice in parallel with this function's own cascade below —
    // tripling the burst of Supabase requests on every login and tripping
    // Supabase's rate limiter (429) on slower/higher-latency connections,
    // which is what left some PCs stuck on the login screen. Removed.
