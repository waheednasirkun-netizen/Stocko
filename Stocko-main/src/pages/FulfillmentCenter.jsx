...
export default function FulfillmentCenter() {
  ...
  const executeDispatch = async (request, item, inv, qty, notes) => {
    ...
    try {
      ...
      // 1. Deduct inventory FIRST — transactions are the source of truth for stock levels
      const { data: txnData, error: stockError } = await transactionsApi.stockOut({
        item: item._displayName,
        qty,
        unit: item._unit || inv?.unit || 'pcs',
        type: 'Fulfillment',
      ...
      if (stockError) {
        console.error('Deduct inventory error:', stockError)
        throw new Error(`Failed to deduct inventory: ${stockError.message}`)
      }
      ...
      const threshold = inv?.minQty || inv?.threshold || inv?.min_stock || 0
      if (newQty <= threshold && threshold > 0) {
        showToast('warning', 'Low Stock Alert', `${item._displayName} reached minimum threshold (${fmtNum(newQty)} ${item._unit})`)
        addNotification?.({
          type: 'warning',
          title: 'Low Stock Alert',
          message: `${item._displayName} reached minimum threshold (${fmtNum(newQty)} ${item._unit})`,
        })
      }
      ...
      // 2. Update request_items fulfilled_qty
      if (item._itemId) {
