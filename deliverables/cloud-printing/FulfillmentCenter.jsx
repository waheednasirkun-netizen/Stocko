...
import {
  PRINT_JOB_TYPES,
  createFulfillmentReceiptPayload,
  enqueuePrintJob,
} from '../lib/printQueue'
...
export default function FulfillmentCenter() {
  ...
  const openDispatchBrowserFallback = useCallback((data, queueError) => {
    console.error(
      '[Fulfillment] Cloud receipt queue failed; opening browser fallback:',
      queueError
    )
    showToast(
      'warning',
      'Printer queue unavailable',
      'The dispatch succeeded, but its receipt was not queued. Opening browser print.'
    )
    setReceiptData(data)
    setTimeout(() => {
      try {
        ...
        console.error('[Fulfillment] Browser receipt fallback failed:', printError)
        showToast('error', 'Browser print failed', 'Use the completed request to try printing again.')
      } finally {
  ...
  const executeDispatch = async (request, item, inv, qty, notes) => {
    ...
    try {
      ...
      // 5. Queue the receipt only after the fulfilment work has succeeded.
      ...
      const nextReceiptData = {
        ...
        branchName: currentBranch?.name || user?.branch_name || request.branch_name || request.branchName,
      }
      try {
        const printBranchId = currentBranch?.id || user?.branch_id || null
        const printPayload = createFulfillmentReceiptPayload({
          request: {
            ...request,
            status: newStatus,
          },
          ...
          timestamp: receiptTimestamp,
          metadata: {
            origin: 'fulfillment_success',
            request_item_id: item._itemId || null,
          },
        })
        const printResult = await enqueuePrintJob({
          branchId: printBranchId,
          jobType: PRINT_JOB_TYPES.FULFILLMENT_RECEIPT,
          source: 'fulfillment',
          payload: printPayload,
        })

        if (printResult.success) {
          showToast('success', 'Print queued', 'The dispatch receipt was sent to the branch printer')
        } else {
          openDispatchBrowserFallback(nextReceiptData, printResult.error)
        }
      } catch (printError) {
        openDispatchBrowserFallback(nextReceiptData, printError)
      }
